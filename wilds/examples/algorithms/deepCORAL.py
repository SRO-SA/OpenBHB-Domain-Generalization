...
class DeepCORAL(SingleModelAlgorithm):
    ...
    def coral_penalty(self, x, y):
        ...
        # print('cent_x: ', cent_x, 'cent_y', cent_y)
        cova_x = (cent_x.t() @ cent_x) / (len(x) - 1+ elipson)
        cova_y = (cent_y.t() @ cent_y) / (len(y) - 1 + elipson)

        # print('mean_x: ', mean_x, 'mean_y: ', mean_y, 'cova_x: ', cova_x, 'cova_y: ', cova_y)
