...
def maximum(numbers, empty_val=0.):
    if isinstance(numbers, torch.Tensor):
        print(numbers)
        if numbers.numel()==0 or numbers.numel()==None:
            return torch.tensor(empty_val, device=numbers.device)
        else:
...
def subsample_idxs(idxs, num=5000, take_rest=False, seed=None):
    """
    Subsamples indices from an array of indices.
    
    Args:
        idxs (numpy.ndarray): Array of indices.
        num (int): Number of indices to subsample.
        take_rest (bool): If True, leaves out the specified number of indices.
        seed (int): Seed for the random number generator.
        
    Returns:
        numpy.ndarray: Subsampled indices.
    """
